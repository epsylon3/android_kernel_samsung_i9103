...
static struct rfkill *bt_sleep_rfkill;
#endif
...
static int bcm4330_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	if (blocked) {
		...
		pr_info("[BT] gpio shutdown=%d, reset=%d, irq=%d (gpio %d)\n",
			bcm4330_rfkill->gpio_shutdown, bcm4330_rfkill->gpio_reset,
			bcm4330_rfkill->irq, TEGRA_IRQ_TO_GPIO(bcm4330_rfkill->irq));
...
#ifdef BT_SLEEP_ENABLE
static int bcm4330_bt_set_sleep(void *data, bool blocked)
{
	if (blocked) {
		pr_debug("[BT] wake_lock(bt_wake_lock)\n");
		gpio_direction_output(bcm4330_rfkill->gpio_btwake, 1);
...
static const struct rfkill_ops bcm4330_btsleep_rfkill_ops = {
	.set_block = bcm4330_bt_set_sleep,
};
#endif

irqreturn_t bt_host_wake_irq_handler(int irq, void *dev_id)
{
	pr_debug("[BT] bt_host_wake_irq_handler\n");
...
static int bcm4330_rfkill_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		...
		pr_err("[BT] unable to register bt_rfkill, ret=%d\n", ret);
		goto free_bcm_irq;
	}

	pr_info("[BT] gpio nshutdown %d, nreset %d, irq bt_host_wake %d (gpio %d)\n",
		bcm4330_rfkill->gpio_shutdown, bcm4330_rfkill->gpio_reset,
		bcm4330_rfkill->irq, TEGRA_IRQ_TO_GPIO(bcm4330_rfkill->irq));
...
static int bcm4330_rfkill_remove(struct platform_device *pdev)
{
	...
	if (bcm4330_rfkill->bt_32k_clk)
		clk_put(bcm4330_rfkill->bt_32k_clk);
#ifdef BT_SLEEP_ENABLE
	rfkill_unregister(bt_sleep_rfkill);
	rfkill_destroy(bt_sleep_rfkill);
	gpio_free(bcm4330_rfkill->gpio_btwake);
#endif
