 ...
 * Galaxy R Tegra adaptation by Tanguy Pruvot <tpruvot@gibhub>
 *
			...
			struct raw_mmc_panic_ops *panic_ops)
{
	...
	blkdev_put(bdev, FMODE_READ);
	pr_info("apanic: Bound to mmc block device %s (%d:%d)\n",
	...
	if (hdr->version != PHDR_VERSION) {
		pr_warning("apanic: Version mismatch (%d != %d)\n",
	...
	pr_notice("apanic: found con(%x, %u) thr(%x, %u) app(%x, %u)\n",
			...
			struct raw_mmc_panic_ops *panic_ops)
{
	...
	pr_info("apanic: memdump backing device set to '%s(%d:%d)',"
...
{
	pr_notice("apanic: calling apanic_mmc via debugfs\n");
	apanic_mmc(NULL, 0, NULL);
	*val = has_apanic_mmc_dump;
	return 0;
}
...
static int __init apanic_mmc_init(void)
{
	...
	if (!ret) {
		...
		pr_err("apanic: %s partition not found on mmc\n",
	...
	if (!ret) {
		pr_info("apanic: %s selected as memdump device\n",
			MEM_DUMP_LABEL);
...
/* should be loaded after mmc partitions parsing */
//late_initcall(apanic_mmc_init); but doesnt work

device_initcall(apanic_mmc_init);
