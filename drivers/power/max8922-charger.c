...
#if defined (CONFIG_MACH_BOSE_ATT)
	chg_ing = stmpe_get_topoff_value();
#else
	...
	if (CHG_EN_USING_TR) {
		...
		if (enable) {
			if (info->is_usb_cable) {
				...
				gpio_set_value(gpio_chg_en, GPIO_LEVEL_HIGH);
			} else {
...
static void max8922_enable_irq_wake(struct max8922_info *info, bool enable)
{
	int ret;
	static bool old_enable=0;
	...
	if(old_enable != enable) {
		if (enable) {
			...
			ret = disable_irq_wake(info->irq_chg_ing);
			if (ret < 0)
				dev_err(info->dev,
					"failed to disable wakeup src %d\n", ret);
		}
		old_enable = enable;
	}
}
...
static irqreturn_t max8922_chg_ing_irq(int irq, void *data)
{
	...
	dev_info(info->dev, "chg_ing(%d) ta_nconnnected(%d)\n", chg_ing, ta_nconnected);
...
#if 0
	ret = gpio_request(pdata->gpio_chg_en, "GPIO_CHG_EN");
	if (ret) {
