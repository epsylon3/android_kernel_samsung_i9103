...
static struct device *sec_kxtf9_dev;

static ssize_t kxtf9_get_data(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct kxtf9_data *kxtf9  = dev_get_drvdata(dev);
...
#ifdef FACTORY_TEST
	sec_kxtf9_dev = device_create(sec_class, NULL, 0, kxtf9,
			"sec_kxtf9");
	if (IS_ERR(sec_kxtf9_dev))
		printk("Failed to create device!");

	if (device_create_file(sec_kxtf9_dev, &dev_attr_kxtf9_rawdata) < 0) {
