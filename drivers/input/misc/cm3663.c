...
static DEVICE_ATTR(proximity_avg, 0664, proximity_avg_show, proximity_avg_store);
...
static void cm3663_work_func_light(struct work_struct *work)
{
	int i;
	int als;
	struct cm3663_data *cm3663 = container_of(work, struct cm3663_data, work_light);
...
irqreturn_t cm3663_irq_thread_fn(int irq, void *data)
{
	...
	printk("%s: proximity value = %d, val = %d\n", __func__, tmp, val);
...
static int cm3663_setup_irq(struct cm3663_data *cm3663)
{
	...
	rc = gpio_request(cm3663->i2c_client->irq, "cm3663_irq");
	if (rc < 0) {
		pr_err("%s: gpio %d request failed (%d)\n",
			__func__, cm3663->i2c_client->irq, rc);
		return rc;
	}

	rc = gpio_direction_input(cm3663->i2c_client->irq);
	if (rc < 0) {
...
err_gpio_direction_input:
	gpio_free(cm3663->i2c_client->irq);
...
err_input_allocate_device_proximity:
	free_irq(cm3663->i2c_client->irq, 0);
err_setup_irq:
err_setup_reg:
...
static int cm3663_i2c_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&cm3663->proximity_input_dev->dev.kobj,
			   &proximity_attribute_group);
	input_unregister_device(cm3663->proximity_input_dev);
	free_irq(cm3663->i2c_client->irq, NULL);
