...
/* woojin80.kim : This is refered from S1.
		...
		enum usb_device_speed speed, void *buf, u8 type)
{
	...
	u8 *dest;
#ifdef CONFIG_USB_ANDROID_SAMSUNG_COMPOSITE
/* wookwang.lee : set interface number dynamically based on product function sequence. */
	int i;
 ...
 */
		...
		if(has_mtp == 1) {
			...
			memcpy(os_desc->wData, &output.qwsignature, sizeof(output.qwsignature));			// code review
	 	...
	 	if(cdev->products) {
			...
			for(i = 0; i < cdev->product_num; i++) {
				if (!strcmp(cdev->products->functions[i], "mtp")) {
					...
					memcpy(os_func_desc->wData, &ms_descriptor, sizeof(ms_descriptor));			// code review
...
composite_switch_work(struct work_struct *data)
{
	...
	if (config)
 	{
 		CSY_DBG_ESS("[composite_switch_work]config=0x%p, config->bConfigurationValue=%d\n",
