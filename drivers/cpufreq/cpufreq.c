...
static ssize_t show_frequency_voltage_table(struct cpufreq_policy *policy, char *buf)
{
	...
	for (i = 0; i < FREQCOUNT; i++)
		table += sprintf(table, "%d %d %d\n", cpufrequency[i], cpuvoltage[i],
			 (cpuvoltage[i]-cpuuvoffset[i]));

	// TODO: Should be frequency, default voltage, current voltage
...
static ssize_t store_UV_mV_table(struct cpufreq_policy *policy, const char *buf, size_t count)
{
	...
	for (i = 0; i < FREQCOUNT; i++)
	{
		...
		if ((cpuvoltage[i]-tmptable[i]) > CPUMVMAX
		 || (cpuvoltage[i]-tmptable[i]) < CPUMVMIN)
			return -EINVAL;
		else
